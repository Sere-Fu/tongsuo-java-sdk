...
import org.conscrypt.SM2ParameterSpec;
import org.conscrypt.SM2PrivateKeySpec;
import org.conscrypt.SM2PublicKeySpec;
import org.conscrypt.SM2SignatureParameterSpec;
...
public class SignatureTest {
    ...
    public void testSign_SM3withSM2_Key_Success() throws Exception {
        ...
        SM2PrivateKeySpec keySpec = new SM2PrivateKeySpec(SM2_S, SM2ParameterSpec.instance());
        PrivateKey privKey = kf.generatePrivate(keySpec);
        ...
        SM2PublicKeySpec pubKeySpec = new SM2PublicKeySpec(SM2_W, SM2ParameterSpec.instance());
        PublicKey pubKey = kf.generatePublic(pubKeySpec);
        sig.initVerify(pubKey);
        sig.update(Vector2Data);
        assertTrue("Signature must verify correctly", sig.verify(signature));
    ...
    @Test
    public void testVerify_SM3withSM2_Wrong_Id_Failure() throws Exception {
        KeyFactory kf = KeyFactory.getInstance("SM2");
        SM2PublicKeySpec pubKeySpec = new SM2PublicKeySpec(SM2_W, SM2ParameterSpec.instance());
        PublicKey pubKey = kf.generatePublic(pubKeySpec);

        Signature sig = Signature.getInstance("SM3withSM2");
        SM2SignatureParameterSpec sm2SignatureParameterSpec = new SM2SignatureParameterSpec("wrong id".getBytes());
        sig.setParameter(sm2SignatureParameterSpec);
        sig.initVerify(pubKey);
        sig.update(Vector2Data);
        assertFalse("Wrong id should not verify", sig.verify(SM3withSM2_Vector2Signature));
    }
